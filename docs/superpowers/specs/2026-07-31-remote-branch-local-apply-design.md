...
기준 브랜치는 로컬 브랜치만 선택할 수 있고 비교 브랜치에는 원격 추적
브랜치를 선택할 수 있다. 하지만 현재는 두 ref가 모두 로컬 브랜치일
때만 적용 버튼이 활성화된다. Git 계층도 두 tip을 모두
`refs/heads/*`에서 찾기 때문에 원격 추적 브랜치를 비교 대상으로
선택하면 기존 적용 흐름을 사용할 수 없다.

## 결정 사항

- 원격 추적 ref는 읽기 전용 입력으로 사용한다.
- Merge는 기존 로컬 기준 브랜치를 변경한다.
- Rebase는 로컬 비교 브랜치를 변경한다. 비교 ref가 원격이면 먼저
  대응하는 로컬 브랜치를 정한다.
...
| --- | --- | --- | --- |
| Merge | 로컬 | 로컬 또는 원격 | 기존 로컬 기준 브랜치 |
| Rebase | 로컬 | 로컬 | 기존 로컬 비교 브랜치 |
| Rebase | 로컬 | 원격 | 비교 ref에 대응하는 로컬 브랜치 |

두 ref가 모두 로컬 브랜치라면 현재 동작을 그대로 유지한다.

### 원격 Rebase 대상을 로컬 브랜치에 대응시키는 방법
...
1. 선택한 로컬 기준 ref와 비교 ref를 확인하고 지금과 같은 방식으로
   미리보기를 계산한다.
2. 미리보기 방식에 따라 쓰기 가능한 변경 대상을 정한다.
3. Rebase 비교 대상이 원격이고 같은 이름의 로컬 브랜치가 다른 SHA에
   있으면 해당 로컬 브랜치로 비교 대상을 바꿔 미리보기를 다시
   계산한다.
...
Merge의 변경 대상은 항상 로컬 기준 브랜치다. 비교 ref는 Merge
부모로만 사용하므로 원격 추적 ref인 상태로 둘 수 있다.
...
Rebase의 변경 대상은 비교 브랜치이며 기준 ref는 항상 로컬이다.
...
- Rebase: `main 위로 feature Rebase 실제 적용`
...
- 원격 비교 ref를 기존 로컬 기준 브랜치에 Merge한다.
