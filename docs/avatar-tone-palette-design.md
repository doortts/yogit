...
### 1. tone 추출 — `AvatarService.toneFor(GitIdentity)` (avatars.dart)

```dart
Future<Color?> toneFor(GitIdentity identity)
```

1. `restore()` (기존 lazy 경로 그대로 — avatars.dart:242)
2. identity의 아바타를 알아낸다. 이미 `_known`에 있으면 재사용, 없으면 `resolve()`가 쓰는
   조회 경로를 identity 하나로 탄다 — 본인 커밋 행이 화면에 그려지길 기다리지 않는다.
...
4. 없으면 이미지에서 계산: `NetworkImage(url, headers:).resolve()` + `ImageStreamListener`
   ...
   enterprise 토큰 헤더도 그대로 따라간다. `ui.Image.toByteData(rawRgba)` → 픽셀 산술평균
   (큰 이미지는 stride로 4096픽셀 이하만 샘플링) → persist → 반환.
5. 조회 실패·디코드 실패·계정 없음 → null. 예외는 밖으로 새지 않는다.
...
Map<int, int> assignBranchPaletteIndexes(
  ...
  List<Color>? refPaletteText,   // 팔레트 text색 (순서 = 팔레트 인덱스)
...
- `_loadCommitIdentity()`(timeline_data.dart:320)가 identity를 확정한 직후:
  `widget.avatarService?.toneFor(identity)` → 완료 시 값이 다르면
  `_rebuild(() { _avatarTone = tone; _rebuildGraph(); })`.
  identity 로드는 이미 initState·저장소 전환·프로필 적용 세 곳에서 다시 돌므로(:693,
  :1030, :1304) 별도 재시도 훅이 필요 없다.
- `_rebuildGraph()`(timeline.dart:1257)에서 `refPaletteText`(widget.refPalette에서 추출)와
  `_avatarTone`을 넘긴다. `AvatarService.branchAssignments` 갱신은 지금 코드 그대로.
...
| 재실행 | restore가 저장된 tone을 즉시 줌 → 첫 그래프부터 조정된 배정, 점멸 없음 |
| 사진 교체 (URL 동일) | 저장값으로 뜬 뒤 라이브 재계산이 다르면 한 번 바뀌고 다시 저장 |
| 신원 미설정 / 계정 없음 / 토큰 없음 | tone null → 오늘과 완전히 동일 |
...
4. **toneFor + 배선**: AvatarService.toneFor, TimelineScreen 연결. 테스트 8·10.
