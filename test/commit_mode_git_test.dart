...
void main() {
  test(
    'loadWorkingTreeStatus reads both sections from a real repository',
    () async {
      ...
      final modified = status.entries.firstWhere(
  ...
  test(
    ...
    () async {
      ...
      expect(
        await runGit(root, ['diff', '--cached', '--name-only']),
        'a.txt\n',
      );
    },
  );
  ...
  test(
    'discarding an untracked symlink deletes the link, not its target',
    () async {
      final root = await createGitFixture();
      addTearDown(() => root.delete(recursive: true));
  ...
  test(
    'commitIndex surfaces identity-missing and pre-commit-hook failures',
    () async {
      final root = await createGitFixture();
      addTearDown(() => root.delete(recursive: true));
      await writeAndCommit(root, 'a.txt', 'one\n', 'base');
  ...
  test(
    'two consecutive stageHunk calls survive because each re-reads the diff',
    () async {
      final root = await createGitFixture();
      addTearDown(() => root.delete(recursive: true));
      await writeAndCommit(root, 'a.txt', _numbered(20), 'base');
  ...
  test(
    'discardHunk refuses a hunk the file rewrote at the same line count',
    () async {
      final root = await createGitFixture();
      addTearDown(() => root.delete(recursive: true));
      await writeAndCommit(root, 'a.txt', _numbered(20), 'base');
  ...
  test('discardHunk removes the hunk from the worktree only', () async {
    ...
    expect(
      await File('${root.path}/a.txt').readAsString(),
      _numbered(20, edits: {10: 'line 10 staged', 17: 'line 17 changed'}),
    );
    expect(
      await _indexBlob(root, 'a.txt'),
  ...
  test(
    'loadAreaDiff(unstaged) compares worktree to index and (staged) index to HEAD',
    () async {
      final root = await createGitFixture();
      addTearDown(() => root.delete(recursive: true));
      await writeAndCommit(root, 'a.txt', 'one\ntwo\nthree\n', 'base');
    ...
    final repository = GitRepository(
      ...
      runner: (executable, arguments, {workingDirectory, environment}) async {
        ...
        return ProcessResult(
          0,
          0,
          arguments.first == 'config' ? 'myers' : '',
          '',
        );
      },
    );
    final area = WorkingTreeAreaRepository(repository, WorkingTreeArea.staged);
