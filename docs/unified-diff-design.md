...
2026-08-09 갱신: 툴바 Full Diff 버튼과 ⌘D 은퇴, pane 이동키(←/→·h/l) 도입 —
`docs/pane-focus-mockup.html`, `docs/preview-header-mockup.html`
...
- 미리보기가 닫혀 있을 때 →를 누르면 미리보기를 먼저 열고, 파일 목록이 도착하면 진입한다.
...
- **진입**: ① 미리보기 파일 목록에서 파일 클릭 ② 타임라인에서 →(또는 l) — 가리키던 파일이
  없으면 첫 파일. 툴바의 Full Diff 버튼과 ⌘D는 은퇴했다(2026-08-09).
- **복귀**: esc · 헤더의 ← · 미리보기에서 →(오른쪽 끝을 벗어나는 방향). 타임라인+사이드바가
  돌아오고 미리보기 pane은 그대로 남는다. 돌아온 목록은 떠날 때 보던 자리에서 다시 시작한다 —
  단, diff에서 History로 커밋을 바꿨다면 그 새 행이 보이는 자리로 간다.
- **파일 이동**: ⌘↑/↓가 미리보기 목록의 선택을 옮기고 diff가 따라온다 (기존 동작 유지).
  diff 내용 스크롤은 ⇧⌘↑/↓.
- **키보드**: ⌘1 Diff · ⌘2 Blame · ⌘3 History · ⌘U 레이아웃 · ⇧A 알고리즘 · ⇧Space 공백 무시 ·
  ⇧L 줄바꿈 · ⇧H Hunk · ⇧F 집중 모드. pane 안에서 ←/→(h/l)가 diff·History·미리보기를 오가고,
  diff에서 ↑↓(j/k)는 hunk를, ⌘↑↓은 파일을 걷는다. ⌘ 누르고 있으면 단축키 힌트도 그대로.
...
8. **복귀**: esc · 헤더의 ← · 미리보기에서 → 로 타임라인+사이드바 복귀, 미리보기 유지.
...
12. **route 부재**: diff 진입이 Navigator push를 하지 않는다 (툴바 버튼과 ⌘D는 은퇴).
